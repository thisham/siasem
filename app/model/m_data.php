    ...
    function master($menu = '', $act = '', $data = '')
    {
        switch ($menu) {
            ...
            case 'kelas':
                switch ($act) {
                    case 'detail':
                        return $this->pdo->kueri("SELECT * FROM $this->dtcls JOIN $this->dttch ON cls_tch = tch_id JOIN $this->dtmjr ON cls_mjr = mjr_id JOIN $this->dtrom ON cls_rom = rom_id WHERE cls_id = :cls_id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;

                    case 'hapus':
                        return $this->pdo->kueri("DELETE FROM $this->dtcls WHERE cls_id = :cls_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;

                    case 'id-add':
                        $no_max = $this->pdo->kueri("SELECT max(cls_id) as kode FROM $this->dtcls")->eksekusi()->hasil_tunggal();
                        $no_max = (int) substr($no_max['kode'], 3); 
                        $no_max = ++$no_max;
                    ...
                    case 'tambah':
                        ...
                        return $this->pdo->kueri("INSERT INTO $this->dtcls VALUES (:cls_id, :cls_name, :cls_tch, :cls_mjr, :cls_rom, :cls_status)")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    ...
                    case 'update':
                        ...
                        return $this->pdo->kueri("UPDATE $this->dtcls SET cls_name = :cls_name, cls_mjr = :cls_mjr, cls_tch = :cls_tch, cls_rom = :cls_rom, cls_status = :cls_status WHERE cls_id = :cls_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    
                    default:
                        return $this->pdo->kueri("SELECT * FROM $this->dtcls JOIN $this->dttch ON cls_tch = tch_id JOIN $this->dtrom ON cls_rom = rom_id")->eksekusi()->hasil_jamak();
                        break;
                }
                break;
    ...
    function user($menu = '', $act = '', $data = '')
    {
        switch ($menu) {
            ...
            case 'guru':
                switch ($act) {
                    case 'detail':
                        return $this->pdo->kueri("SELECT * FROM $this->dttch WHERE tch_id = :tch_id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;
                    ...
                    case 'photo-upload':
                        $this->pdo->kueri("UPDATE $this->dtusr SET usr_photo = :tch_photo WHERE usr_id = :tch_id")->ikat($data)->eksekusi()->baris_terefek();
                        return $this->pdo->kueri("UPDATE $this->dttch SET tch_photo = :tch_photo WHERE tch_id = :tch_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    ...
                    case 'tambah':
                        ...
                        return $this->pdo->kueri("INSERT INTO $this->dttch VALUES (:tch_id, :tch_idnumber, :tch_name, :tch_passworddef, :tch_gender, :tch_birthplace, :tch_birthdate, :tch_nik, :tch_nuptk, :tch_est, :tch_ptk, :tch_fieldofstudy, :tch_religion, :tch_streetaddress, :tch_rt, :tch_rw, :tch_hamlet, :tch_village, :tch_district, :tch_postalcode, :tch_phone, :tch_email, :tch_additionalassignment, :tch_status, :tch_foundationacc, :tch_foundationaccdate, :tch_salarysource, :tch_mothername, :tch_maritalstatus, :tch_maritalpartner, :tch_maritalpartnernik, :tch_maritalpartnerjob, :tch_npwp, :tch_nationality, :tch_photo)")
            ...
            case 'kepala-sekolah':
                switch ($act) {
                    case 'update':
                        ...
                        return $this->pdo->kueri("UPDATE $this->dthdm SET hdm_idnumber = :hdm_idnumber, hdm_name = :hdm_name, hdm_passworddef = :hdm_passdef, hdm_address = :hdm_address, hdm_phone = :hdm_phone, hdm_email = :hdm_email, hdm_position = :hdm_position WHERE hdm_id = :hdm_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    
                    default:
                        $data = array('id' => '20202001');
                        return $this->pdo->kueri("SELECT * FROM $this->dthdm WHERE hdm_id = :id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;
                }
                break;
            ...
            case 'user':
                switch ($act) {
                    ...
                    case 'tambah-hdm':
                        ...
                        return $this->pdo->kueri("UPDATE $this->dtusr SET usr_name = :usr_name, usr_password = :usr_pass, usr_role = :usr_role, usr_photo = :usr_photo, usr_status = :usr_status WHERE usr_id = :usr_id")->ikat($data)
                                         ->eksekusi()->baris_terefek();
                        break;
    ...
    function akademik($menu = '', $act = '', $data = '')
    {
        switch ($menu) {
            case 'kelompok-mapel':
                switch ($act) {
                    case 'detail':
                        return $this->pdo->kueri("SELECT * FROM $this->dtsgr WHERE sgr_id = :sgr_id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;

                    case 'hapus':
                        return $this->pdo->kueri("DELETE FROM $this->dtsgr WHERE sgr_id = :sgr_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    ...
                    case 'tambah':
                        return $this->pdo->kueri("INSERT INTO $this->dtsgr VALUES (:sgr_id, :sgr_group, :sgr_name)")->ikat($data)->eksekusi()->baris_terefek();
                        break;

                    case 'update':
                        return $this->pdo->kueri("UPDATE $this->dtsgr SET sgr_group = :sgr_group, sgr_name = :sgr_name WHERE sgr_id = :sgr_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    
                    default:
                        return $this->pdo->kueri("SELECT * FROM $this->dtsgr")->eksekusi()->hasil_jamak();
                        break;
                }
                break;
            ...
            case 'jadwal':
                switch ($act) {
                    case 'detail':
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dtrom ON cls_rom = rom_id WHERE ssc_id = :ssc_id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;

                    case 'hapus':
                        return $this->pdo->kueri("DELETE FROM $this->dtssc WHERE ssc_id = :ssc_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    ...
                    case 'list':
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dttch ON ssc_tch = tch_id JOIN $this->dtrom ON cls_rom = rom_id WHERE ssc_fys = :ssc_fys AND ssc_cls = :ssc_cls AND ssc_day = :ssc_day AND ssc_status = :ssc_stt")->ikat($data)->eksekusi()->hasil_jamak();
                        break;

                    case 'list-guru':
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dttch ON ssc_tch = tch_id JOIN $this->dtrom ON cls_rom = rom_id WHERE ssc_tch = :ssc_tch")->ikat($data)->eksekusi()->hasil_jamak();
                        break;

                    case 'list-hari':
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dttch ON ssc_tch = tch_id JOIN $this->dtrom ON cls_rom = rom_id WHERE ssc_day = :ssc_day")->ikat($data)->eksekusi()->hasil_jamak();
                        break;

                    case 'list-kelas':
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dttch ON ssc_tch = tch_id JOIN $this->dtrom ON cls_rom = rom_id WHERE ssc_cls = :ssc_cls ORDER BY ssc_timestart ASC")->ikat($data)->eksekusi()->hasil_jamak();
                        break;

                    case 'non-aktif':
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dttch ON ssc_tch = tch_id JOIN $this->dtrom ON cls_rom = rom_id WHERE ssc_status = 'off'")->eksekusi()->hasil_jamak();
                        break;
                    ...
                    case 'tambah':
                        ...
                        return $this->pdo->kueri("INSERT INTO $this->dtssc VALUES (:ssc_id, :ssc_fys, :ssc_cls, :ssc_sbj, :ssc_tch, :ssc_timestart, :ssc_timeend, :ssc_day, :ssc_status)")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    ...
                    case 'update':
                        ...
                        $data['ssc_timeend'] = date('H:i:s', strtotime($data['ssc_timeend']));
                        $data['ssc_status'] = (isset($data['ssc_status'])) ? 'on' : 'off';
                        return $this->pdo->kueri("UPDATE $this->dtssc SET ssc_fys = :ssc_fys, ssc_cls = :ssc_cls, ssc_sbj = :ssc_sbj, ssc_tch = :ssc_tch, ssc_timestart = :ssc_timestart, ssc_timeend = :ssc_timeend, ssc_day = :ssc_day, ssc_status = :ssc_status WHERE ssc_id = :ssc_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    
                    default:
                        return $this->pdo->kueri("SELECT * FROM $this->dtssc JOIN $this->dtfys ON ssc_fys = fys_id JOIN $this->dtcls ON ssc_cls = cls_id JOIN $this->dtsbj ON ssc_sbj = sbj_id JOIN $this->dtrom ON cls_rom = rom_id JOIN $this->dttch ON ssc_tch = tch_id")->eksekusi()->hasil_jamak();
                        break;
                }
                break;
            ...
            case 'mapel':
                switch ($act) {
                    case 'detail':
                        return $this->pdo->kueri("SELECT * FROM $this->dtsbj JOIN $this->dttch ON sbj_tch = tch_id JOIN $this->dtcur ON sbj_cur = cur_id JOIN $this->dtsgr ON sbj_sgr = sgr_id JOIN $this->dtmjr ON sbj_mjr = mjr_id WHERE sbj_id = :sbj_id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;

                    case 'hapus':
                        return $this->pdo->kueri("DELETE FROM $this->dtsbj WHERE sbj_id = :sbj_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    ...
                    default:
                        return $this->pdo->kueri("SELECT * FROM $this->dtsbj JOIN $this->dttch ON sbj_tch = tch_id")->eksekusi()->hasil_jamak();
                        break;
                }
                break;
            ...
            case 'rentang-nilai':
                switch ($act) {
                    case 'detail':
                        return $this->pdo->kueri("SELECT * FROM $this->dtgrd WHERE grd_id = :grd_id")->ikat($data)->eksekusi()->hasil_tunggal();
                        break;

                    case 'hapus':
                        return $this->pdo->kueri("DELETE FROM $this->dtgrd WHERE grd_id = :grd_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;

                    case 'id-add':
                        $no_max = $this->pdo->kueri("SELECT max(grd_id) as kode FROM $this->dtgrd")->eksekusi()->hasil_tunggal();
                        $no_max = (int) substr($no_max['kode'], 3);
                        $no_max = ++$no_max;
                    ...
                    case 'tambah':
                        return $this->pdo->kueri("INSERT INTO $this->dtgrd VALUES (:grd_id, :grd_min, :grd_max, :grd_grade, :grd_information)")->ikat($data)->eksekusi()->baris_terefek();
                        break;

                    case 'update':
                        return $this->pdo->kueri("UPDATE $this->dtgrd SET grd_min = :grd_min, grd_max = :grd_max, grd_grade = :grd_grade, grd_information = :grd_information WHERE grd_id = :grd_id")->ikat($data)->eksekusi()->baris_terefek();
                        break;
                    
                    default:
                        return $this->pdo->kueri("SELECT * FROM $this->dtgrd")->eksekusi()->hasil_jamak();
                        break;
                }
                break;
